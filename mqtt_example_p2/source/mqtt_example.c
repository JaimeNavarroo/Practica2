...
#define EXAMPLE_MQTT_USER "JaimeLeonel"
#define EXAMPLE_MQTT_PSWD "aio_OFid50CiW0qQrmUZMawtjqFtYGd7"
...
#define MQTT_FAN_EVT			( 1 << 2 )
#define MQTT_DISCONNECTED_EVT	( 1 << 3 )
#define MQTT_PUB2_EVT	( 1 << 4 )
#define MQTT_PUB3_EVT	( 1 << 5 )
#define MQTT_LED_EVT	( 1 << 6 )
...
static void publish_sensor_data(void *ctx);
...
uint32_t cpu_temperature = 25;
uint32_t cpu_clock_speed = 100;
uint32_t ram_usage = 50;
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    if(!memcmp(data, "On", 2)) {
    	cpu_fan = true;
    	xEventGroupSetBits(xEventGroup,	MQTT_FAN_EVT);
    }
    ...
    else if(!memcmp(data, "Red", 3)) {
    	red_led = true;
    	green_led = false;
    	blue_led = false;
    	xEventGroupSetBits(xEventGroup,	MQTT_LED_EVT);
    }

    else if(!memcmp(data, "Green", 5)) {
    	red_led = false;
    	green_led = true;
    	blue_led = false;
    	xEventGroupSetBits(xEventGroup,	MQTT_LED_EVT);
...
{
    static const char *topics[] = {"JaimeLeonel/feeds/project2.subscriber1",
    							   "JaimeLeonel/feeds/project2.subscriber2"};
    int qos[]                   = {1,1};
    err_t err;
    int i;
...
static void publish_sensor_data(void *ctx)
{
    static const char *topic1   = "JaimeLeonel/feeds/project2.publisher1";
    static const char *topic2   = "JaimeLeonel/feeds/project2.publisher2";
    static const char *topic3   = "JaimeLeonel/feeds/project2.publisher3";
    static char message[10];

    LWIP_UNUSED_ARG(ctx);

    memset(message, 0, 10);
    sprintf(message, "%d", cpu_temperature);

    PRINTF("Going to publish to the topic \"%s\"...\r\n", topic1);

    mqtt_publish(mqtt_client, topic1, message, strlen(message), 1, 0, mqtt_message_published_cb, (void *)topic1);

    memset(message, 0, 10);
    sprintf(message, "%d", cpu_clock_speed);

    PRINTF("Going to publish to the topic \"%s\"...\r\n", topic2);

    mqtt_publish(mqtt_client, topic2, message, strlen(message), 1, 0, mqtt_message_published_cb, (void *)topic2);

    memset(message, 0, 10);
    sprintf(message, "%d", ram_usage);

    PRINTF("Going to publish to the topic \"%s\"...\r\n", topic3);

    mqtt_publish(mqtt_client, topic3, message, strlen(message), 1, 0, mqtt_message_published_cb, (void *)topic3);
}
...
static void app_thread(void *arg)
{
    ...
    xTimerSensor = xTimerCreate("TimerSns", 2000 / portTICK_PERIOD_MS, pdTRUE, (void*)&timerId, sensor_timer_callback);
    ...
    /* Turn OFF the LEDs */
	GPIO_PortSet(BOARD_LED_GPIO_B, 1U << BOARD_LED_GPIO_PIN_B);
	GPIO_PortSet(BOARD_LED_GPIO_G, 1U << BOARD_LED_GPIO_PIN_G);
	GPIO_PortSet(BOARD_LED_GPIO_R, 1U << BOARD_LED_GPIO_PIN_R);
    ...
    while(1) {
		...
		uxBits = xEventGroupWaitBits(
					...
					MQTT_CONNECTED_EVT | MQTT_SENSOR_EVT | MQTT_FAN_EVT | MQTT_DISCONNECTED_EVT |
					MQTT_PUB2_EVT | MQTT_PUB3_EVT | MQTT_LED_EVT ,	// The bits within the event group to wait for.
		...
		if(uxBits & MQTT_CONNECTED_EVT ) {
			...
			// Simulate the CPU temperature and the clock speed, depending if the fan is on or not
			cpu_clock_speed = get_simulated_sensor(cpu_clock_speed, 4, 85, 120, cpu_fan);
			cpu_temperature = get_simulated_sensor(cpu_temperature, 2, 20, 60, !cpu_fan);

			if((samples_cnt++%10) == 9){
				err = tcpip_callback(publish_sensor_data, NULL);
				if (err != ERR_OK)
				{
					PRINTF("Failed to invoke publish_sensor_data on the tcpip_thread: %d.\r\n", err);
				}
			}
		}
		else if(uxBits & MQTT_LED_EVT ) {
			PRINTF("MQTT_LED_EVT.\r\n");
			if(red_led){
				/* Turn ON the RED LED. */
				GPIO_PortClear(BOARD_LED_GPIO_R, 1U << BOARD_LED_GPIO_PIN_R);
				...
				GPIO_PortClear(BOARD_LED_GPIO_G, 1U << BOARD_LED_GPIO_PIN_G);
			...
			else if(blue_led)
			{
				...
				GPIO_PortClear(BOARD_LED_GPIO_B, 1U << BOARD_LED_GPIO_PIN_B);
...
int main(void)
{
    ...
    PRINTF("\r\n************************************************\r\n");
    PRINTF(" Project 2 - System monitor dashboard demo\r\n");
    PRINTF("************************************************\r\n");
